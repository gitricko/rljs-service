...
// Enhancing JQuery with PUT and DELETE
...
Agent.prototype = {
  ...
  getSuccessRate: function() {
    return (this.apples / this.poison).toFixed(3);
  },
  setEpsilon: function(value) {
    var spec = {'epsilon':value};
    $.put( './agents/' + this.id + '/spec', {spec: JSON.stringify(spec)});
    this.epsilon = value;
  },
  ...
  saveBrain: function(callback) {
    $.post( './agents/' + this.id + '/model/db_save', {
        success: this.getSuccessRate()
      }, typeof callback === 'function' && callback());
  },
  loadPretrainedBrain: function(env, spec, callback) {
    $.post( './agents/' + this.id + '/loadPretrainedBrain', {
        agent: JSON.stringify(env),
        spec: JSON.stringify(spec)
      }, callback);
  },
  loadBrain: function(env, spec, brainID, callback) {
   $.post( './db_models/' + brainID, {
        agent: JSON.stringify(env),
        spec: JSON.stringify(spec),
        id: this.id
      }, callback);
  },
  deleteBrain: function(brainID) {
    $.delete( './db_models/' + brainID);
  },
  fetchBrains: function(callback) {
    $.get( './db_models', callback);
  },
  initBrain: function(env, spec, callback) {
    ...
    $.post( './agents',
      { agent: JSON.stringify(env),
        ...
        numActions: env.getMaxNumActions()
      },
        initFunction);
  },
  forward: function(callback) {
    ...
    var actFunction = (function( data ) {
          this.action = parseInt(data);
          callback();
    }).bind(this);
    $.post( './agents/' + this.id + '/act',
      { state: JSON.stringify(input_array)
      }, actFunction);
  ...
  backward: function(callback) {
    ...
    $.post( './agents/' + this.id + '/learn', { reward: reward}, callback);
  }
};
